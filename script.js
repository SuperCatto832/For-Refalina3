...
button.addEventListener("click", () => {
  if (button.textContent === "Click Me! ❤") {
    ...
    fetch('send_mail.php')
      .then(response => {
        if (response.ok) {
          button.textContent = "From your beloved Yahya Aditya ❤️";
        } else {
...
function drawText() {
    ...
    // glow effect
    context.shadowColor = "rgb(180, 0, 255)";
    context.shadowBlur = 8;
    context.shadowOffsetX = 0;
    context.shadowOffsetY = 0;

    if(frameNumber < 250){
        context.fillStyle = `rgba(180, 0, 255, ${opacity})`;
    ...
    if(frameNumber > 2500 && frameNumber < 99999){
        ...
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks(["I love you so much Refalina, more than", "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText("I love you so much Refalina, more than all the time and space in the universe can contain", canvas.width/2, canvas.height/2);
        }
    ...
    if(frameNumber >= 2750 && frameNumber < 99999){
        context.fillStyle = `rgba(180, 0, 255, ${secondOpacity})`;
    ...
    if(frameNumber >= 3000 && frameNumber < 99999){
        context.fillStyle = `rgba(180, 0, 255 ${thirdOpacity})`;
        context.fillText("Happy Valentine's Day Sayangku Refalina <3", canvas.width/2, (canvas.height/2 + 120));
